...
\subsection*{Nebenaspekte der Effizienz} 
	...
	$\rhd$ Heutzutage z.B. auch bei größeren Datenmengen noch relevant \\
	$\rhd$ Begriffe: \\
	...
	\hspace{0.4cm} $\diamond$ Nur wenige Quelltext is für den Großteil der Ineffizienz verantwortlich \\
...
\subsection*{Hauptaspekt der Effizient - Laufzeit}
	...
	\hspace{0.4cm} $\diamond$ Immer feste Größe an Bytes zwischen Cache und Hauptspeicher transferiert \\
	...
	\hspace{0.6cm} - Ist um Größenordnungen schneller, aber nicht portabel } \\ \hline
	...
	\hspace{0.6cm} - In Kennzahlen, die die Problemgröße beschreiben \\
	...
	\hspace{0.4cm} $\diamond$ $f \in o(g)$ und $f \in \Theta(g)$ schließen sich logisch aus \\
	...
	\hspace{0.4cm} $\diamond$ $\Theta$ induziert eine Äquivalenzrelation, das heißt, es gilt: \\
	...
	\hspace{0.4cm} $\diamond$ $O$ induziert eine partielle Ordnung bzgl. $\Theta$, das heißt, es gilt: \\
...
\subsection*{Fehlersuche}
	...
	$\rhd$ Fehler in Bibliothekskomponente eher auszuschließen \\
	$\rhd$ Falls undurchschaubar: verdächtigen Quelltext neu implementieren} \\ \hline  

	\makecell[l]{Laufzeittests} & \makecell[l]{
	$\rhd$ Black-Box-Tests \\
	\hspace{0.4cm} $\diamond$ Testen einer Klasse von außen, ohne hineinzuschauen \\
...
\subsection*{Fehlervermeidender Entwurf}
	...
	\hspace{0.4cm} $\diamond$ Der Kopf der überschriebenen Methode darf nur im engen Maße abweichen \\
	...
	\hspace{0.4cm} $\diamond$ Methode in Subklasse macht dann nichts außer diese \texttt{Exception} zu werfen \\
...
\subsection*{Generelle Einteilung} 
	...
	$\rhd$ \texttt{Racket}: \\
	\hspace{0.4cm} $\diamond$ Funktionen, die auf verschiedenen Typen mit versch. Operationen arbeiten können \\
	\hspace{0.6cm} - z.B. \texttt{foldr}, \texttt{map}, ... \\
	$\rhd$ \texttt{Java}: \\
	\hspace{0.4cm} $\diamond$ Ableitungen von Klassen / Implementierung von Interfaces \\
	\hspace{0.6cm} - Polymorphie hier: Speicherung eines Subtyp-Objekts in Referenz des Basistyps \\
...
\subsection*{Abstraktion und Polymorphie}
	...
	\hspace{0.6cm} - \texttt{Class<?> c = obj.getClass();} \\
	\hspace{0.6cm} - $\rightarrow$ \textbf{dynamische Prüfung} } \\ \hline
